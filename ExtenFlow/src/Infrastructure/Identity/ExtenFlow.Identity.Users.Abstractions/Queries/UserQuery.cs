...
namespace ExtenFlow.Identity.Users.Queries
{
    ...
    public class UserQuery<T> : Query<T>
    {
        ...
        public UserQuery(string aggregateId, string userId, Guid? correlationId = null, Guid? messageId = null, DateTimeOffset? dateTime = null)
            : base(nameof(User), aggregateId, userId, correlationId ?? Guid.NewGuid(), messageId ?? Guid.NewGuid(), dateTime ?? DateTimeOffset.Now)
        {
        }
    }
}