...
namespace ExtenFlow.Identity.Roles
{
    ...
    public class RoleQuery<T> : Query<T>
    {
        ...
        public RoleQuery(string aggregateId, string userId, Guid? correlationId = null, Guid? messageId = null, DateTimeOffset? dateTime = null)
            : base(nameof(Role), aggregateId, userId, correlationId ?? Guid.NewGuid(), messageId ?? Guid.NewGuid(), dateTime ?? DateTimeOffset.Now)
        {
        }
    }
}