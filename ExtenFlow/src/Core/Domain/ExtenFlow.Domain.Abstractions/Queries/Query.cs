...
namespace ExtenFlow.Domain
{
    ...
    public abstract class Query<T> : Request, IQuery<T>
    {
        ...
        protected Query(string aggregateType, string aggregateId, string userId, string? correlationId = null, string? id = null, DateTimeOffset? dateTime = null) : base(aggregateType, aggregateId, userId, correlationId, id, dateTime)
        {
        }
    }
}