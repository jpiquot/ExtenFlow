...
namespace ExtenFlow.Identity.IdentityServer
{
    ...
    public class AccountController : Controller
    {
        ...
        /// </summary>
        [HttpGet]
        public async Task<IActionResult> Login(string returnTo)
        {
            // build a model so we know what to show on the login page
            LoginViewModel? vm = await BuildLoginViewModelAsync(returnTo.ToUri());

            if (vm.IsExternalLoginOnly)
            {
                // we only have one option for logging in and it's an external provider
                return RedirectToAction("Challenge", "External", new { provider = vm.ExternalLoginScheme, returnTo });
            }
        ...
        public async Task<IActionResult> Login(LoginInputModel model, string button)
        {
            ...
            AuthorizationRequest? context = await _interaction.GetAuthorizationContextAsync(model.ReturnUrl?.ToString());
            ...
            if (button != "login")
            {
                if (context != null)
                {
                    ...
                    {
                        // if the client is PKCE then we assume it's native, so this change in how
                        // to return the response is for better UX for the end user.
                        return this.LoadingPage("Redirect", model.ReturnUrl?.ToString());
                    }
            ...
            if (ModelState.IsValid)
            {
                ...
                if (_users.ValidateCredentials(model.Username, model.Password))
                {
                    ...
                    // only set explicit expiration here if user chooses "remember me". otherwise we
                    // rely upon expiration configured in cookie middleware.
                    ...
                    if (Url.IsLocalUrl(model.ReturnUrl?.ToString()))
                    {
                        return Redirect(model.ReturnUrl?.ToString());
                    }
                    else if (string.IsNullOrEmpty(model.ReturnUrl?.ToString()))
                    {
                        return Redirect("~/");
                    }
                    else
                ...
                await _events.RaiseAsync(new UserLoginFailureEvent(model.Username, Properties.Resources.InvalidCredentials, clientId: context?.Client.ClientId));
        ...
        private async Task<LoggedOutViewModel> BuildLoggedOutViewModelAsync(string? logoutId)
        {
            ...
            var vm = new LoggedOutViewModel
            {
                ...
                PostLogoutRedirectUri = logout?.PostLogoutRedirectUri.ToUri(),
                ClientName = string.IsNullOrEmpty(logout?.ClientName) ? logout?.ClientId : logout?.ClientName,
                SignOutIframeUrl = logout?.SignOutIFrameUrl.ToUri(),
                LogoutId = logoutId
            };
        ...
        private async Task<LoginViewModel> BuildLoginViewModelAsync(Uri? returnUrl)
        {
            AuthorizationRequest? context = await _interaction.GetAuthorizationContextAsync(returnUrl?.ToString());
